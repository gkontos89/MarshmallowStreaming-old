...
class SlingBlue(StreamingPackage):
    def __init__(self):
        super().__init__("Sling Blue")
        self.web_driver = webdriver.Chrome()

    def scrape_for_channels(self):
        try:
            self.web_driver.get('https://www.sling.com/#dyn-grid-plan-tab-2-s-2')
    ...
    def scrape_for_price(self):
        try:
            self.web_driver.get('https://www.sling.com/#dyn-grid-plan-tab-2-s-2')
            plan_tab = self.web_driver.find_element_by_xpath("//a[@href='#dyn-grid-plan-tab-2-s-2']")
