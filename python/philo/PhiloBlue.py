...
class PhiloBlue(StreamingPackage):
    ...
    def scrape_for_channels(self):
        ...
        # PhiloBlue is Green plus Blue
        green_lineup = self.web_driver.find_elements_by_class_name('lineup-ud95d')[0]
        blue_lineup = self.web_driver.find_elements_by_class_name('lineup-ud95d')[1]
        self.price = blue_lineup.find_element_by_class_name('price-1M_hC').text
        channels = green_lineup.find_elements_by_class_name('channel-SzvkI') + \
            blue_lineup.find_elements_by_class_name('channel-SzvkI')
